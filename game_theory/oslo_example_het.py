...
def check_equilibrium(prices, tput, total_ct, x_lo, x_hi, min_jobs, max_jobs):
    ...
    return True, equilibrium_profile
...
def get_equilibrium(prices, n_players):
    ...
    for next_job_ct in range(1, sum(max_jobs)+2):
        ...
        if is_valid and sum(x_lo) <= (next_job_ct-1) <= sum(x_hi):
            ...
            found_eq, Q_per_player = check_equilibrium(prices, tputs, next_job_ct-1, x_lo, x_hi, min_jobs, max_jobs)
    ...
    return found_eq, Q, Q_per_player,eq_tput
...
if __name__ == "__main__":
    ...
    for run_no in range(RUNS_PER_COUNT):
        base_price = (random.random() * 0.8) + 0.2
        #base_price = base_price ** 0.5
        #prices = [random.random()**0.5 for _ in range(n_players)]
        get_random = lambda: min(max(base_price + np.random.normal(0, 0.1), 0.2), 1)
        prices = [get_random() for _ in range(n_players)]

        is_valid, Q, Q_per_player, eq_tput = get_equilibrium(prices, n_players)

        if not is_valid:
            n_invalid += 1
            print("found invalid. Total invalid: ", n_invalid)
            print(f"percent invalid: {n_invalid/run_no}")
            continue

        Qs.append(Q)
        Qs_per_player.append(Q_per_player)
        eq_tputs.append(eq_tput)
        player_counts.append(n_players)
        all_prices.append(prices)
        base_prices.append(base_price)

        print(f"Q per player: {Q_per_player}")
    ...
    with open(filename, "w") as csvfile:
        ...
        for i in range(len(Qs)):
            writer.writerow([player_counts[i], all_prices[i], base_prices[i], Qs[i], Qs_per_player[i], eq_tputs[i]])
